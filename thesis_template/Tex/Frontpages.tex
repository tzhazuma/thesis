...
\englishauthor{Tian Zihang}
\englishadvisor{Supervisor: Prof. XXX}% 指导教师
...
本研究构建了面向膜体系的分子动力学模拟原型系统，实现了三种长程静电求解器：直接求和参考解（$O(N^2)$）、PPPM基线方法和RBSOG方法。通过系统化的基准测试，从计算性能、统计方差和结构稳定性三个维度对PPPM和RBSOG进行了全面比较。基于本文重新串行执行的真实实验结果，RBSOG在当前纯Python原型中的平均每步时间显著高于PPPM，在10个随机种子统计下约为其5.47倍；同时，RBSOG的压力方差均值也高于PPPM，说明该方法在当前实现阶段尚未展现出统计稳定性优势。不过，批量大小扫描与JIT消融实验表明，RBSOG对实现细节和参数选择较为敏感，且JIT加速可带来约2.32倍的内核级提速，这表明该方法仍具有进一步工程优化空间。
...
We constructed a membrane-oriented molecular dynamics simulation prototype system implementing three types of long-range electrostatics solvers: direct pairwise Coulomb reference ($O(N^2)$), PPPM baseline, and the proposed RBSOG method. Through systematic benchmarking, we comprehensively compared PPPM and RBSOG across three dimensions: computational performance, statistical variance, and structural stability. Based on the final serially executed experiments in this thesis, RBSOG remains substantially slower than PPPM in the current pure-Python prototype and also shows higher pressure variance in the 10-seed benchmark. Nevertheless, batch-size sweep and just-in-time compilation ablation results indicate that the method is highly sensitive to implementation details and still has room for optimization. Furthermore, we introduced a weighted Pareto frontier analysis method for automated batch size parameter $P$ recommendation and established a comprehensive evaluation system including run-level statistics, effect size analysis, and JIT ablation experiments.
...
\englishkeywords{Molecular Dynamics Simulation, Long-Range Electrostatics, Sum-of-Gaussians, Random Batch Sampling, Whole-Cell Simulation, Membrane System}% 英文关键词
%---------------------------------------------------------------------------%
